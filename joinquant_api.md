...
- 描述：获取历史数据，可查询多个标的多个数据字段，返回数据格式为 DataFrame
...
- 描述：获取历史数据，可查询多个标的单个数据字段，返回数据格式为 DataFrame 或 Dict(字典)
...
- 描述：获取历史数据，可查询单个标的多个数据字段，返回数据格式为 DataFrame 或 Dict(字典)
...
- 描述：获取历史数据(包含快照数据)，可查询单个或多个标的多个数据字段，返回数据格式为 numpy.ndarray或DataFrame
...
- 描述：获取股票、期货、50ETF期权、股票指数及场内基金的 tick 数据
...
- 描述：用以对组合总权重进行限制，可设置一个或多个相同/不同类别的函数
...
- 描述：用以对组合中单标的权重进行限制，可设置一个或多个相同/不同类别的函数
...
#### **初始化融资融券账户**<span id="setrzrq"></span>
